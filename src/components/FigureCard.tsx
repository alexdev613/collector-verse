...
export function FigureCard({ figure }: Props) {
  ...
  return (
    ...
    >
      ...
      <Link
        to={`/figures/${figure.id}/edit`}
        onClick={(e) => e.stopPropagation()}
        className="absolute z-10 bg-white/80 hover:bg-white w-8 h-8 rounded-full flex items-center justify-center right-2 top-2 drop-shadow border border-border"
      >
        <FaPencil size={12} color="#222" />
      </Link>

      {/* IMAGE */}
      <Link to={`/figures/${figure.id}`} className="block">
